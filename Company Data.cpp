...
public:
    ...
    void searchInt(string type) { //can b
        Employee* empList[numEmployees];
        int index = 0;
        treeToArray(empList, root, index);

        if (type == "Age") { //start sorting by Age
            int age;
    ...
    void searchStr(string type) {
        ...
        if (type == "Job Title") { //start sorting by job title name
            string title;
        ...
        if (type == "Employee ID") { //start sorting by employee ID
            string emID;
...
int main() {
    ...
    do {
        ...
        case 0:
            break;
        case 1:
            cout << "Enter Job Position of new employee's BOSS: " << endl;
            cin.ignore();
